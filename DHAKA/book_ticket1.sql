                ...
                ticket_booked_id := ticket_booked_id + 1;
                    ...
                    IF (Sourcee = 'Dhaka') THEN
                        ...
                        FOR J IN (SELECT * FROM Train WHERE Source=Sourcee AND Destination=Destinationn) LOOP
                            ...
                            --DBMS_OUTPUT.PUT_LINE(max_ticket_cap);
                            current_train_no := J.Train_no;
                            IF ((max_ticket_cap<least_seat)) THEN
                    ...
                    ELSIF (Sourcee = 'Chittagong') THEN
                        ...
                        FOR J IN (SELECT * FROM Train@site3 WHERE Source=Sourcee AND Destination=Destinationn) LOOP
                            max_ticket_cap := J.Seat_Left;
                            current_train_no := J.Train_no;
                            IF ((max_ticket_cap<least_seat) ) THEN
                                least_seat := max_ticket_cap;
                        ...
                        INSERT into TicketBooked@site3 values(ticket_booked_id,seat_noo,final_train_no,p_id);
                        
                        UPDATE Ticket@site3 SET TCap=(Tcap-seat_noo) WHERE Train_no=final_train_no;

                        UPDATE Train@site3 SET Seat_Left=(Seat_Left-seat_noo) WHERE Train_no=final_train_no;
