...
BEGIN
    ...
    IF (passenger_active_status = 1) THEN
        FOR R IN (SELECT Train.Train_Name, TicketBooked.TAmount FROM Train@site1 INNER JOIN TicketBooked@site1 ON Train.Train_no=TicketBooked.Train_no WHERE TicketBooked.PID=passenger_id) LOOP
